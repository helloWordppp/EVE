...
def setup_pct(package_path, language, start=True):
    """
    静默安装pct
    ...
    print(system_root)
    tim_begin = time.time()
    pct_main_path_x64 = os.path.join(system_root, r"Program Files (x86)\EaseUS\EaseUS Todo PCTrans\bin\PCTrans.exe")
    pct_main_path_x86 = os.path.join(system_root, r"Program Files\EaseUS\EaseUS Todo PCTrans\bin\PCTrans.exe")
    while True:
    ...
    if start:
        ...
        while True:
            pct_main = get_pid_by_name("PCTrans")
            if pct_main:
                break
            time.sleep(10)
    return True
...
def clear_environment(app_name=None):
    ...
    :return: None
    """
    kill_list = ["_iu14D2N.tmp", "unins000.exe", "PCTrans.exe", "uetool.exe", "uexperice.exe"]
